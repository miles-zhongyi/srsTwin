...
#include <cstdlib>
#include <cstring>   // for std::strcmp, std::getenv
#include <ctime>
...
void rrc::send_con_request(srsran::establishment_cause_t cause)
{
  ...
  // TELUS 22_decoded trace injection for 4G LTE path.
  // RRC_TRACE_LTE_M_TMSI  — decimal uint32 m_tmsi extracted from 22_decoded record
  //                         by rrc_trace_fields.py via rrc_injector_entrypoint.sh.
  // RRC_TRACE_LTE_CAUSE   — LTE establishmentCause string (e.g. "mo-Signalling").
  // When unset the original behaviour (ue_identity_configured check / random) is used.
  const char* trace_m_tmsi = std::getenv("RRC_TRACE_LTE_M_TMSI");
  const char* trace_cause  = std::getenv("RRC_TRACE_LTE_CAUSE");

  if (trace_m_tmsi != nullptr && trace_m_tmsi[0] != '\0') {
    // Override UE identity: use real TELUS m_tmsi as randomValue (40-bit).
    uint64_t random_id = std::strtoull(trace_m_tmsi, nullptr, 10) & 0xFFFFFFFFULL;
    rrc_conn_req->ue_id.set_random_value();
    rrc_conn_req->ue_id.random_value().from_number(random_id);
    logger.info("RRC Conn Request: TELUS trace LTE m_tmsi=%s (random_value=%llu)",
                trace_m_tmsi, (unsigned long long)random_id);
  } else if (ue_identity_configured) {
  ...
  if (trace_cause != nullptr && trace_cause[0] != '\0') {
    // Map the LTE establishmentCause string to the srsRAN enum value.
    struct { const char* str; establishment_cause_opts::options val; } cause_map[] = {
      {"emergency",          establishment_cause_opts::emergency},
      {"highPriorityAccess", establishment_cause_opts::high_prio_access},
      {"mt-Access",          establishment_cause_opts::mt_access},
      {"mo-Signalling",      establishment_cause_opts::mo_sig},
      {"mo-Data",            establishment_cause_opts::mo_data},
      {"delayTolerantAccess",establishment_cause_opts::delay_tolerant_access_v1020},
    };
    for (const auto& m : cause_map) {
      if (std::strcmp(trace_cause, m.str) == 0) {
        cause = (srsran::establishment_cause_t)m.val;
        logger.info("RRC Conn Request: TELUS trace LTE cause=%s", trace_cause);
        break;
      }
    }
  }
