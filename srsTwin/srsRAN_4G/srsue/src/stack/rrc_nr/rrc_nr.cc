...
void rrc_nr::send_setup_request(srsran::nr_establishment_cause_t cause)
{
  ...
  // Use real TELUS subscriber identity when available.
  // RRC_TRACE_M_TMSI  — decimal uint32 m_tmsi from 22_decoded record (set by
  //                     rrc_injector_entrypoint.sh via rrc_trace_fields.py).
  // RRC_TRACE_CAUSE   — LTE/NR establishment cause string (e.g. "mo-Signalling").
  // When unset, fall back to srsRAN's internal random value (identical transmit path).
  const char* trace_m_tmsi = std::getenv("RRC_TRACE_M_TMSI");
  const char* trace_cause  = std::getenv("RRC_TRACE_CAUSE");

  if (trace_m_tmsi != nullptr && trace_m_tmsi[0] != '\0') {
    // m_tmsi is a 32-bit value; NR randomValue is 39 bits — fits with room to spare.
    uint64_t random_id = std::strtoull(trace_m_tmsi, nullptr, 10);
    rrc_setup_req->ue_id.random_value().from_number(random_id,
        rrc_setup_req->ue_id.random_value().length());
    logger.info("RRC Setup Request: TELUS trace m_tmsi=%s (39-bit random_value=%llu)",
                trace_m_tmsi, (unsigned long long)random_id);
  } else {
    // Original behaviour: random identity.
  ...
  if (trace_cause != nullptr && trace_cause[0] != '\0') {
    // Map the LTE cause string to the matching NR establishment_cause enum value.
    // Both standards use the same enumeration order (emergency=0 … spare1=15).
    struct { const char* str; srsran::nr_establishment_cause_t val; } cause_map[] = {
      {"emergency",          srsran::nr_establishment_cause_t::emergency},
      {"highPriorityAccess", srsran::nr_establishment_cause_t::highPriorityAccess},
      {"mt-Access",          srsran::nr_establishment_cause_t::mt_Access},
      {"mo-Signalling",      srsran::nr_establishment_cause_t::mo_Signalling},
      {"mo-Data",            srsran::nr_establishment_cause_t::mo_Data},
      {"mo-VoiceCall",       srsran::nr_establishment_cause_t::mo_VoiceCall},
      {"mo-VideoCall",       srsran::nr_establishment_cause_t::mo_VideoCall},
      {"mo-SMS",             srsran::nr_establishment_cause_t::mo_SMS},
      {"mps-PriorityAccess", srsran::nr_establishment_cause_t::mps_PriorityAccess},
      {"mcs-PriorityAccess", srsran::nr_establishment_cause_t::mcs_PriorityAccess},
    };
    for (const auto& m : cause_map) {
      if (std::strcmp(trace_cause, m.str) == 0) {
        cause = m.val;
        logger.info("RRC Setup Request: TELUS trace cause=%s", trace_cause);
        break;
      }
    }
  }
